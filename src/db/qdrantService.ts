    ...
    try {
      ...
      if (existingCollection) {
        this.loggingService.info(
          ...
          { vectorSize: existingCollection.config?.params?.vectors?.size },
          "QdrantService",
        );

        // Validate that existing collection has correct vector size
        const existingVectorSize = existingCollection.config?.params?.vectors?.size;
        if (existingVectorSize && existingVectorSize !== vectorSize) {
          this.loggingService.warn(
            `Collection '${collectionName}' exists but has different vector size`,
            { expected: vectorSize, actual: existingVectorSize },
            "QdrantService",
          );
        }
